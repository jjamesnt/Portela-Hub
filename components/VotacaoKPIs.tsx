...
const VotacaoKPIs: React.FC<VotacaoKPIsProps> = ({ municipioId, codigoIBGE, totalRecursos, selectedMandato }) => {
    ...
    const marildaCard = (
        <div key="marilda" className={`rounded-xl p-4 md:p-5 shadow-lg shadow-orange-500/10 relative overflow-hidden border border-orange-500/40 bg-gradient-to-br from-orange-500/15 via-white to-white dark:from-orange-500/25 dark:via-slate-800 dark:to-slate-800 transition-all hover:shadow-xl hover:shadow-orange-500/20 hover:scale-[1.01] ${selectedMandato !== 'Marilda Portela' && selectedMandato !== 'Todos' ? 'opacity-50 grayscale scale-[0.98]' : ''}`}>
            <div className="absolute -bottom-3 -right-3 opacity-[0.07]">
                <span className="material-symbols-outlined text-[60px] md:text-[100px] text-orange-500">person</span>
            </div>
            <div className="flex items-center gap-1.5 md:gap-2 mb-1.5 md:mb-2 text-left">
                <div className="flex items-center justify-center w-5 h-5 md:w-6 md:h-6 rounded bg-orange-500 shadow-sm shadow-orange-500/40 text-left">
                    <span className="material-symbols-outlined text-white text-[12px] md:text-sm text-left">groups</span>
                </div>
                <p className="text-[8px] md:text-[10px] font-black text-slate-600 dark:text-slate-300 uppercase tracking-wider leading-tight text-left">Vereadora<br className="md:hidden" /> Marilda</p>
            </div>
            <h3 className="text-xl md:text-3xl font-black text-orange-500 mt-0.5 md:mt-1 tracking-tight">
    ...
    if (selectedMandato === 'Lincoln Portela') {
        orderedCards.push(lincolnCard, aleCard, marildaCard);
    } else if (selectedMandato === 'Alê Portela') {
        orderedCards.push(aleCard, lincolnCard, marildaCard);
