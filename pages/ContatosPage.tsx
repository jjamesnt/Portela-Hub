...
const ContatosPage: React.FC<{ navigateTo: (page: string, params?: any) => void }> = ({ navigateTo }) => {
    ...
    // Reseta a página sempre que os filtros mudarem
    useEffect(() => {
        setCurrentPage(1);
    }, [selectedMandato, filterType, searchTerm]);
        ...
        <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6 pb-24">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden">
                ...
                <div className="p-4 border-t border-slate-100 dark:border-slate-700 flex justify-between items-center text-xs font-medium text-slate-500">
                    <div>Exibindo {paginatedContacts.length} de {filteredContacts.length} contatos</div>
                    <div className="flex items-center gap-1">
                        <button 
                            onClick={() => setCurrentPage(p => Math.max(1, p - 1))}
                            className="size-7 rounded bg-slate-50 hover:bg-slate-100 flex items-center justify-center disabled:opacity-50" 
                            disabled={currentPage === 1}
                        >
                            <span className="material-symbols-outlined text-[16px]">chevron_left</span>
                        </button>
                        <button className="px-3 h-7 rounded bg-turquoise text-white flex items-center justify-center font-bold">
