                ...
                <div className="space-y-8">
                    ...
                    {isLoading ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
                            {briefings.length === 0 ? (
                                <div className="col-span-full bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-8 text-center text-slate-500 border border-dashed border-slate-300 dark:border-slate-700">
                                    Nenhuma pauta ou briefing publicado no momento.
                                </div>
                            ) : (
                                briefings.map(briefing => (
                                    <div key={briefing.id} className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200/60 dark:border-slate-700/50 flex flex-col overflow-hidden hover:-translate-y-1 hover:shadow-xl hover:border-slate-300 dark:hover:border-slate-600 transition-all duration-300 group cursor-pointer">
                                        <div className="flex justify-between items-center px-5 py-4 border-b border-slate-100 dark:border-slate-700/50">
                                            <span className={`text-[10px] font-black uppercase tracking-widest flex items-center gap-2 ${briefing.origem.includes('Marilda') ? 'text-pink-600 dark:text-pink-400' : 'text-slate-600 dark:text-slate-300'}`}>
                                                <div className={`w-2 h-2 rounded-full ${briefing.origem.includes('Marilda') ? 'bg-pink-600 dark:bg-pink-400' : 'bg-slate-600 dark:bg-slate-400'}`}></div> {briefing.origem.split(' ')[0]}
                                            </span>
