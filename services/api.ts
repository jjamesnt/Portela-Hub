...
export const getBriefings = async (origem?: string): Promise<Briefing[]> => {
    try {
        const query = origem && origem !== 'Geral' ? `WHERE origem = '${origem}' OR origem = 'Geral'` : '';
        const response = await apiClient.post<any>('/api/admin/sql', {
            sql: `SELECT * FROM hub.briefings ${query} ORDER BY data_publicacao DESC, created_at DESC LIMIT 20`
        });
...
export const createBriefing = async (briefing: Partial<Briefing>): Promise<Briefing | null> => {
    try {
        const response = await apiClient.post<any>('/api/admin/sql', {
            sql: `INSERT INTO hub.briefings (titulo, descricao, acao_sugerida, prioridade, origem) 
                  VALUES ('${briefing.titulo}', '${briefing.descricao || ''}', '${briefing.acao_sugerida || ''}', '${briefing.prioridade || 'MÉDIA'}', '${briefing.origem}') RETURNING *`
        });
        return response?.rows?.[0] as Briefing || null;
    } catch (error) {
